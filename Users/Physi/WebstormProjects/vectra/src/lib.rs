...
#[cfg(target_arch = "wasm32")]
type Vectra = crate::db::Vectra<filesystem::web::DirectoryHandle>;
...
pub mod native {
    ...
    pub type Db = Vectra<crate::filesystem::native::DirectoryHandle>;
}
...
pub mod memory {
    ...
    pub type Db = Vectra<DirectoryHandle>;
}
...
#[wasm_bindgen]
pub struct Db {
    Vectra: crate::db::Vectra<filesystem::web::DirectoryHandle>,
}
...
impl Db {
    ...
    pub async fn new() -> Self {
        ...
        let Vectra = Vectra::new(file_system_directory_handle);
    ...
    pub async fn insert(&mut self, content: &str, embedding: &[f64], tags: Option<Vec<JsValue>>) {
        ...
        self.Vectra.write(content, embedding, tags).await;
    }
    ...
    pub async fn clear(&mut self) {
        ...
        let result = self.Vectra.clear_db().await; // ignore the error if there is one
        if !result.is_ok() {
            console_warn!("Failed to clear Vectra data: {:?}", result);
        }
    }
}
